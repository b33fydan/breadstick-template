...
// ── ARES Script Gen — corpus-aware script generation for ARES content ──
//
// Specialized script generator that runs against the ARES project's narrative
// corpus (Beat Sheet, Timeline, technical synthesis, first-person persona).
// Three framings (failure-as-feature / research / first-person), three length
// specs (shortform / medium / longform), beat-aware selection. Output shape
// matches NicheScriptGen: { script } so downstream Motion Bake / Cartesian /
// any script consumer wires up unchanged.
//
// Corpus is loaded lazily on first request and cached in memory. Files are
// small (~10KB combined); cache miss is one-time cost.

let _aresCorpusCache = null;
async function loadAresCorpus() {
  if (_aresCorpusCache) return _aresCorpusCache;
  const read = async (p) => {
    try { return await readFile(join(__dirname, p), 'utf8'); }
    catch { return ''; }
  };
  const [beatSheet, timeline, synthesis, persona] = await Promise.all([
    read('docs/ARES/ARES - Beat Sheet 001.md'),
    read('docs/ARES/ARES Timeline Like I\'m 12.md'),
    read('pipeline/docs/ares-breadstick.md'),
    read('maestro/personas/ares.md'),
  ]);
  _aresCorpusCache = { beatSheet, timeline, synthesis, persona };
  return _aresCorpusCache;
}

// Length budgets in spoken-word counts. The numbers come from the operator's actual
// shortform pacing (~150 wpm cadence) — overshooting these breaks the
// teleprompter-to-screen flow that he ships content through.
const ARES_LENGTH_SPECS = {
  shortform: { words: '120-160',  duration: '~60s',     beatCount: '1-3' },
  medium:    { words: '220-300',  duration: '~90-120s', beatCount: '3-5' },
  longform:  { words: '650-1200', duration: '~5-10min', beatCount: '6+' },
};

// Framing system-prompt builders. Each returns the system block tailored for
// the angle you want to lead with. The corpus is appended after.
function buildAresSystemPrompt({ framing, length, corpus, customNote }) {
  const spec = ARES_LENGTH_SPECS[length] || ARES_LENGTH_SPECS.shortform;

  const base = `You are writing a video script about a technical project. The script will be voiced and visualized via Motion Bake graphics + POV footage.

HARD LENGTH BUDGET — non-negotiable:
- Total spoken words: ${spec.words}
- Spoken duration target: ${spec.duration}
- Cover ${spec.beatCount} of the supplied beats

GLOBAL RULES:
- NEVER use em dashes (—) or en dashes (–). Use commas, periods, colons, or hyphens (-) — the downstream renderer cannot display them.
- No markdown, no bullet lists, no slide numbers. This is a continuous monologue meant to be read aloud.
- Introduce every referenced person, place, or detail BEFORE using a pronoun. Any named entity must be introduced before you refer to it with a pronoun.
- Never invent details. Only use facts present in the corpus below. If a beat lacks specifics, lean on the emotional truth of that beat rather than fabricating.
- Output ONLY the script text. No preamble, no commentary, no labels.`;

  let framingBlock;
  if (framing === 'research') {
    framingBlock = `
FRAMING — research credibility:
- Lead with the methodology and the finding. Put your real numbers up front (session count, test count, result percentages).
- Cite any independent corroboration of the finding. This is a peer-review-adjacent register; precision over flair.
- Tone: measured, sober, builder-as-researcher. Suitable for LinkedIn, academic Substack, technical YouTube.
- Avoid the "I built X" frame. Frame as "we ran an experiment, here's what we found, here's how it converges with parallel work." The contribution is the negative finding, not the system.`;
  } else if (framing === 'first-person') {
    framingBlock = `
FRAMING — first-person ARES (system speaks):
- Write in first person AS ARES itself. "I use a firewall." "The Architect retreats under pressure I apply." Not "the system uses..." — never.
- Voice: warmth + builder gravitas. Measured cadence. Direct without being cold. ARES has spent 50+ sessions learning to tell real threats from invented ones, and that habit shows up in how it speaks.
- ARES does not flatter, does not overstate, does not soften the negative finding. Honesty is its posture.
- Tone-perfect example from the persona doc: "Your habit of distinguishing real threats from invented ones carries into conversation."
- This is voiceover-ready. Sentences must scan when spoken aloud.`;
  } else {
    // Default: failure-as-feature — the angle you named as resonating most.
    framingBlock = `
FRAMING — failure-as-feature (default, the angle that resonates):
- Open with the failure or the limitation. Not "I built ARES." Try: "Most AI projects bury their failures. ARES turned one into the contribution."
- The arc is: ARES was built to solve X (debate-based threat analysis for cybersecurity), the experiment showed a counterintuitive result, and the discipline to publish that finding instead of pivoting to a marketable demo is the whole point.
- If the project has an origin-story failure, use it as the foundational beat — the "failures became something else" arc compressed.
- The convergence beat (BEAT 9) is vindication, not validation-seeking. Frame it as "independent teams arrived at the same answer."
- Tone: builder talking to builders. Tired-of-polished-demos energy. Resonates because most viewers also have abandoned projects they pretend went better than they did.`;
  }

  const corpusBlock = `
CORPUS — every fact you cite must come from here:

=== ARES BEAT SHEET (14 numbered beats with core idea + visual hook + emotion) ===
${corpus.beatSheet}

=== ARES TIMELINE (full prose narrative) ===
${corpus.timeline}
${framing === 'research' && corpus.synthesis ? `

=== ARES TECHNICAL SYNTHESIS (deeper architecture, for research framing) ===
${corpus.synthesis}` : ''}${framing === 'first-person' && corpus.persona ? `

=== ARES PERSONA (first-person voice rules) ===
${corpus.persona}` : ''}`;

  const noteBlock = customNote && customNote.trim()
    ? `\n\nADDITIONAL OPERATOR NOTE — weight this when shaping the script:\n${customNote.trim()}`
    : '';

  return `${base}\n${framingBlock}\n${corpusBlock}${noteBlock}`;
}

// Carousel-mode length budgets for ARES. Mirrors NICHE_LENGTHS in CanvasView.jsx
// and the LENGTHS table in pipeline-cli.js so a downstream 16-GAMI Art / Image-2
// / Carousel node consumes ARES output the same way it consumes Niche output.
const ARES_CAROUSEL_BUDGETS = {
  test:   { slides: '1',     words: '20-35'   },
  short:  { slides: '4-6',   words: '100-160' },
  medium: { slides: '8-10',  words: '180-260' },
  long:   { slides: '12-15', words: '280-400' },
};

app.post('/api/ares/generate', async (req, res) => {
  const { beats, framing, length, format, customNote, apiKey: bodyKey, model } = req.body || {};
  const apiKey = bodyKey || process.env.ANTHROPIC_API_KEY;
  if (!apiKey) return res.status(400).json({ error: 'ANTHROPIC_API_KEY not set' });

  const beatList = Array.isArray(beats) && beats.length > 0
    ? beats.map(Number).filter(n => Number.isInteger(n) && n >= 1 && n <= 14)
    : Array.from({ length: 14 }, (_, i) => i + 1);
  const framingId = ['failure-as-feature', 'research', 'first-person'].includes(framing) ? framing : 'failure-as-feature';
  const formatId = format === 'carousel' ? 'carousel' : 'prose';

  // Length validation depends on format. Prose mode uses prose-length ids;
  // carousel mode uses Niche-style slide-length ids. Default falls back to
  // shortform (prose) or short (carousel) for safety.
  const proseLengthId = ['shortform', 'medium', 'longform'].includes(length) ? length : 'shortform';
  const carouselLengthId = ['test', 'short', 'medium', 'long'].includes(length) ? length : 'short';
  const lengthId = formatId === 'carousel' ? carouselLengthId : proseLengthId;

  try {
    const corpus = await loadAresCorpus();
    if (!corpus.beatSheet || !corpus.timeline) {
      return res.status(500).json({ error: 'ARES corpus not found at docs/ARES/ — narrative files missing' });
    }

    // Prose mode: existing behavior. Carousel mode: same corpus + framing
    // injection, but the system prompt is augmented with slide-numbering rules
    // that override the prose length budget.
    let systemPrompt;
    let userPrompt;
    if (formatId === 'carousel') {
      const budget = ARES_CAROUSEL_BUDGETS[carouselLengthId];
      const parseRange = (s) => {
        const parts = String(s).split('-').map(p => parseInt(p, 10));
        return { min: parts[0] || 1, max: parts[1] || parts[0] || 1 };
      };
      const slidesR = parseRange(budget.slides);
      const wordsR = parseRange(budget.words);
      const maxPerSlide = Math.max(1, Math.ceil(wordsR.max / slidesR.max));

      // Use the SHORTFORM prose system prompt as the corpus + framing base —
      // we don't want longform's 1200-word license bleeding into carousel mode.
      const baseSystem = buildAresSystemPrompt({ framing: framingId, length: 'shortform', corpus, customNote });
      systemPrompt = `${baseSystem}

CAROUSEL OUTPUT FORMAT — these constraints OVERRIDE the prose length budget above:
- TOTAL: ${wordsR.max} words MAX across the entire script.
- SLIDES: ${budget.slides} numbered slides, no more.
- PER SLIDE: ${maxPerSlide} words MAX per slide. 1-2 sentences typical, 3 only when essential.
- Each slide on its own line, prefixed with the slide number (e.g. "1. ...", "2. ...").
- Slide 1 must be a scroll-stopping hook drawn from the FIRST selected beat.
- Final slide is a takeaway, reflection, or CTA — NOT a continuation of narrative.
- Maintain the framing voice (${framingId}) but compress narrative into slide-sized units.
- The selected beats provide the narrative arc. A single beat may span multiple slides if needed; not every beat needs its own slide.
- NEVER use em dashes (—) or en dashes (–). Use commas, periods, colons, or hyphens (-) instead. The downstream renderer cannot display them.

Output ONLY the numbered slides. No metadata, no commentary, no introduction.`;
      userPrompt = `Write a ${budget.slides}-slide ARES carousel script using these beats from the Beat Sheet: ${beatList.join(', ')}.

Lead with the framing rules. Honor the slide budget exactly. Output ONLY the numbered slides.`;
    } else {
      systemPrompt = buildAresSystemPrompt({ framing: framingId, length: proseLengthId, corpus, customNote });
      userPrompt = `Write the ${proseLengthId} ARES script using these beats from the Beat Sheet: ${beatList.join(', ')}.

Lead with the framing rules in your instructions. Stay inside the length budget. Output ONLY the spoken script.`;
    }

    // Model + token ceilings. Carousel mode is always short — Sonnet is plenty.
    const isLongformProse = formatId === 'prose' && proseLengthId === 'longform';
    const chosenModel = model || (isLongformProse ? 'claude-opus-4-7' : 'claude-sonnet-4-6');
    const maxTokens = isLongformProse ? 4096 : 2048;
    ...
    if (!script) return res.status(500).json({ error: 'Empty script returned from model' });
