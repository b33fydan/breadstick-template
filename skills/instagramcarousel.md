# Instagram Carousel Generator for Claude Code

A complete pipeline that turns a topic or YouTube URL into branded Instagram carousel slides, rendered as PNGs using Python/Pillow. One command, fully automated.

```
/carousel Why every business needs an AI chatbot
/carousel https://youtube.com/watch?v=abc123
```
...
You give Claude a topic (or a YouTube URL to repurpose). Claude researches the topic, writes the carousel content, sources relevant images, generates a config file, and renders branded 1080x1350 slide PNGs. You review and approve before anything is finalized.

**Pipeline:**
1. **INPUT** - Parse topic string or pull YouTube transcript
2. **RESEARCH** - Web search for context, stats, and images
3. **WRITE** - Structure content into slides (hook, body, CTA)
4. **REVIEW** - Present slide plan for your approval
5. **CONFIG** - Generate config.json + download reference images
6. **RENDER** - Python/Pillow generates branded PNGs
7. **FINAL REVIEW** - Show rendered slides, iterate if needed
...
```
carousels/
├── render.py              # Pillow render script (provided below)
├── assets/
│   ├── headshot.jpg       # Your profile picture (for CTA slide)
│   └── fonts/
│       ├── BebasNeue-Regular.ttf    # Free from Google Fonts
│       ├── Inter.ttc                # Free from Google Fonts (or InterVariable.ttf)
│       └── Caveat.ttf               # Free from Google Fonts
└── workspace/             # Generated carousels go here
    └── <carousel-name>/
        ├── config.json
        ├── reference/     # Source images for this carousel
        └── slide_1.png    # Rendered output
```

### 2. Install Dependencies

```bash
pip install Pillow youtube-transcript-api
```

### 3. Download Fonts

All fonts are free from Google Fonts:
- [Bebas Neue](https://fonts.google.com/specimen/Bebas+Neue) - condensed headlines
- [Inter](https://fonts.google.com/specimen/Inter) - body text (get the .ttc or variable .ttf)
- [Caveat](https://fonts.google.com/specimen/Caveat) - handwritten annotations

### 4. Add Your Headshot

Drop a square-ish photo of yourself at `carousels/assets/headshot.jpg`. This appears on the CTA (last) slide.

---

## The Slash Command

Save this as `.claude/commands/carousel.md` in your project:
...
Save this as `carousels/render.py`:
...
The entire visual identity is controlled by the `COLORS` dictionary at the top of `render.py`. To match your brand:

```python
COLORS = {
    "bg_dark": "#0D1B2A",          # Slide background
    "accent": "#5DCCFF",            # Text highlights, bullet dots, underlines
    "accent_secondary": "#E91E63",  # CTA button, gradient bars
    "white": "#FFFFFF",             # Headlines
    "off_white": "#F0F0F0",         # Body text
    "gray": "#94A3B8",              # Secondary text (handles, subtitles)
    "blue": "#1976D2",              # Gradient bar left side
    "red": "#E91E63",               # Gradient bar right side
    "grid_line": "#1A2D42",         # Subtle grid overlay
}
```
...
Here's what a generated `config.json` looks like:

```json
{
  "title": "5 AI Tools Every Business Needs in 2025",
  "profile": {
    "display_name": "Your Name",
    "handle": "@yourhandle"
  },
  ...
  "slides": [
    {
      "type": "hook",
      "text": "5 AI tools that replaced my entire team's workflow",
      "subtitle": "And saved 20+ hours per week.",
      "image": "hero-screenshot.jpg"
    },
    {
      "type": "body",
      "title": "THE PROBLEM",
      "text": "Most teams are still doing everything manually. Content, reporting, onboarding. All of it.",
      "bullets": [
        "Content creation takes *hours* every day",
    ...
    },
    {
      "type": "body",
      "title": "TOOL #1",
      "text": "ChatGPT for first drafts. Not the final product, just the starting point.",
      "image": "chatgpt-screenshot.png",
      "annotation": "the 80/20 of content"
    },
    {
      "type": "body",
      "title": "THE SHIFT",
      "text": "You don't need a bigger team. You need better tools.",
      "bullets": [
    ...
    {
      "type": "cta",
      "text": "Follow for more AI automation tips.",
      "button_text": "Follow for more"
    }
  ]
}
...
- **Dimensions:** 1080x1350px (Instagram max portrait)
- **Format:** PNG
- **Background:** Deep navy with radial glow + grid texture
- **Top bar:** 6px gradient (blue to magenta)
- **Footer:** @handle on left, bookmark + "save for later" on right

---

## How It Works Under the Hood

1. **Claude writes content** following the slide structure rules (hook, body slides, CTA)
2. **Claude sources images** via web search and downloads them to `reference/`
3. **Claude generates config.json** with all slide data, text, and image references
4. **render.py reads config.json** and generates each slide using Pillow:
   - Creates the dark navy background with glow + grid texture
   - Renders text using the font stack (Bebas Neue for headlines, Inter for body, Caveat for annotations)
   - Handles `*accent*` syntax for colored keyword highlighting
   - Places and sizes images with rounded corners and drop shadows
   - Draws curved arrows for annotations
   - Adds the editorial footer to every slide
5. **Output PNGs** are saved alongside the config in the workspace directory

---

## Tips

- The hook slide image covers the top ~62% of the slide and fades into the background. Use a wide, detailed image.
- Use `*asterisks*` around 1-2 keywords per slide for visual emphasis. Don't overdo it.
- Keep bullet lists to 3-4 items max. Each bullet should be one short line.
- The `asset:` prefix lets you reference logos and icons in the shared `assets/` folder across all carousels.
- Annotations (handwritten text with arrows) add personality. Use them on 2-3 slides, not every slide.
- The CTA slide pulls your headshot from `assets/headshot.jpg` and your name/handle from config.json.
